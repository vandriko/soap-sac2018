...
%
 ...
 \todo{create new account, update URL if necessary, update activeUserCount, add a couple of sentences of the current state of the case study}
...
%  
  ...
  allows for a custom route (\code{/mymonitor}) to the dashboard to be defined by the programmer. An external configuration file can be used instead/in addition to this:
  ...
  \todo{Add here integration with Travis info as a prerequisite for discussion in Section~\ref{sec:testing}}
...
\label{sec:perf}
  ...
  \todo{rephrase the following, the discussion has been partially subsumed by Section~\ref{sec:tool}.}
